...
typedef helikopterExercise4_feedback_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * helikopterExercise4_feedback(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(helikopterExercise4_feedback_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://helikopterExercise4_feedback:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: helikopterExercise4_feedback -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  S = helikopterExercise4_feedback();
  if (rtmGetErrorStatus(S) != NULL) {
